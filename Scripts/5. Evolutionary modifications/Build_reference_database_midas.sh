...
#According to github page, we need following files: indir: Path to directory of genomes. Each subdirectory should be named with a genome identifier. Each subdirectory should contain the following files; <genome_id>.fna: Genomic DNA sequence in FASTA format; <genome_id>.faa: Protein sequences in FASTA format; <genome_id>.ffn: Gene sequences in FASTA format; <genome_id>.genes: Tab delimited file with genomic coordinates of genes. The file should be tab-delimited file with a header and the following fields:	gene_id (CHAR); scaffold_id (CHAR);	start (INT); end (INT); strand (+ or -); 	gene_type (CDS or RNA); mapfile: Path to mapping file that specifies which genomes belonging to the same species.
#The file should be tab-delimited file with a header and 3 fields:	genome_id (CHAR): corresponds to subdirectory within INDIR; species_id (CHAR): : species identifier for genome_id; rep_genome (0 or 1): indicator if genome_id should be used for SNP calling
