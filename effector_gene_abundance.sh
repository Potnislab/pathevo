#############################################################################################
#################         ShortBred abundaance of type three effectors       ################
##############################################################################################
...
add export path in your script
...
## b. A file containing the genes specific to your species of interest and in the other file all the gene sequences present in other species of genus. (getting all other sequenes can be done by blasting the gene sequences from species of interest on NCBI website)
...
shortbred_identify.py --goi /path/to/gene_of_interest_sequence.fna --ref /path/to/othersequences.fna --markers /path/to/output.faa --usearch ~/bin/usearch6.1.544_i86linux32
...
for F in /path/to/*trimmed_kneaddata.paired.1.fastq;    ## got from the script mentioned in https://github.com/Potnislab/AtDep_2021_metagenome/blob/main/Raw%20read%20processing/host-contamination-removal_script 
do
N=$(basename $F trimmed_kneaddata.paired.1.fastq) ;
shortbred_quantify.py --markers /path/to/all_markers_sequences_geneInterest.faa --wgs $F  --results $N.txt --tmp $N --threads 2 --usearch ~/path/bin/usearch6.1.544_i86linux32
done
...
##install the hclust2 on hpsc (https://github.com/SegataLab/hclust2)
...
##normalize the values for each sample by dividing the abundance of all genes with abundance of one conserved gene 
## (in our case with Avrbs2 normalization done)

##make your dataset by including first two rows with metadata 
